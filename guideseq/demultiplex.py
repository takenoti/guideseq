...
def demultiplex(
    ...
    for r1, r2, i1, i2 in zip(fq(read1), fq(read2), fq(index1), fq(index2)):
        ...
        if total_count % 1000000 == 0:
            logger.info(
                "Processed %d reads in %.1f minutes.",
                total_count,
                (time.time() - start) / 60,
            )
        ...
        if sample_id not in outfiles_r1:
            outfiles_r1[sample_id] = open(
                os.path.join(out_dir, "%s.r1.fastq" % sample_id), "w"
            )
    ...
    logger.info(
        "Wrote FASTQs for the %d sample barcodes out of %d with at least %d reads.",
        num_fastqs,
        len(count),
        min_reads,
    )
    ...
    if subsample_reads > 0:
        ...
        for sample_id in count:
            ...
            command_R1 = "seqtk sample -s100 %s %s > %s" % (
                input_R1,
                subsample_reads,
                out_R1,
            )
            command_R2 = "seqtk sample -s100 %s %s > %s" % (
                input_R2,
                subsample_reads,
                out_R2,
            )
...
def reformat_fastq(
    read1, read2, index1, index2, sample_name, out_dir="out_dir", subsample_reads=-1
):
    ...
    if subsample_reads > 0:
        ...
        if subsample_reads > total_count:
            logger.info(
                "User input subsample_reads:%s is higher than total reads Sample: %s, %s"
                % (subsample_reads, sample_name, total_count)
            )
