...
def get_parameters(manifest_data):
    ...
    # init
    default_refseqName = (
        os.path.dirname(os.path.realpath(__file__)) + "/refseq_gene_name.py"
    )
    ...
    default["Manhattan.R"] = (
        os.path.dirname(os.path.realpath(__file__)) + "/Manhattan.R"
    )
    ...
    return_dict["control_coord_start"], return_dict["control_coord_end"] = [
        int(x) for x in tmp.split("-")
    ]
...
def assign_default(return_dict, default):
    ...
    for k in section_list:
        return_dict[k]["out_dir"] = os.path.join(
            return_dict["analysis_folder"], return_dict[k]["out_dir"]
        )
        return_dict[k]["input_dir"] = os.path.join(
            return_dict["analysis_folder"], return_dict[k]["input_dir"]
        )
...
class GuideSeq:
        ...
        try:
            ...
            logger.info(
                ...
                + tabulate(
                    [
                        (k, v)
                        for k, v in self.samples[list(self.samples.keys())[0]].items()
                    ]
                )
            )
    ...
    def demultiplex(self):
        ...
        # print (barcode_dict)
        is_file_flag = os.path.join(
            self.parameters["demultiplex"]["input_dir"],
            self.parameters["demultiplex"]["forward"],
        )
        ...
        try:
            if os.path.isfile(
                os.path.join(
                    self.parameters["demultiplex"]["input_dir"],
                    self.parameters["demultiplex"]["controlforward"],
                )
            ):
            ...
            if is_file_flag:
                if is_2_Undetermined_samples:
                    ...
                    demultiplex(
                        ...
                        os.path.join(
                            self.parameters["demultiplex"]["input_dir"],
                            self.parameters["demultiplex"]["reverse"],
                        ),
                        os.path.join(
                            self.parameters["demultiplex"]["input_dir"],
                            self.parameters["demultiplex"]["index1"],
                        ),
                        os.path.join(
                            self.parameters["demultiplex"]["input_dir"],
                            self.parameters["demultiplex"]["index2"],
                        ),
                        barcode_dict,
                        ...
                        subsample_reads=self.parameters["demultiplex"][
                            "subsample_reads"
                        ],
                    )
                ...
                logger.info(
                    "Assuming individual fastq files are specified in barcode1, barcode2, controlbarcode1, and controlbarcode2"
                )
    ...
    def demultiplex2(self):
        ...
        try:
            ...
            demultiplex(
                ...
                os.path.join(
                    self.parameters["demultiplex"]["input_dir"],
                    self.parameters["demultiplex"]["controlreverse"],
                ),
                os.path.join(
                    self.parameters["demultiplex"]["input_dir"],
                    self.parameters["demultiplex"]["controlindex1"],
                ),
                os.path.join(
                    self.parameters["demultiplex"]["input_dir"],
                    self.parameters["demultiplex"]["controlindex2"],
                ),
                control_barcode_dict,
                out_dir=self.parameters["demultiplex"]["out_dir"],
                mismatch=self.parameters["demultiplex"]["mismatch"],
    ...
    def add_UMI_to_fastq_name(self):
        ...
        try:
            logger.info(
                "Reformating fastq files...The output files will be in the demultiplex folder"
            )
            for sample in self.samples:
                ...
                try:
                    ...
                    if not "read1" in self.samples[sample]:
                        if self.parameters["demultiplex"]["subsample_reads"] > 0:
                            self.samples[sample]["read1"] = os.path.join(
                                self.parameters["demultiplex"]["out_dir"],
                                f"{sample}.sampling.r1.fastq",
                            )
                            self.samples[sample]["read2"] = os.path.join(
                                self.parameters["demultiplex"]["out_dir"],
                                f"{sample}.sampling.r2.fastq",
                            )
                            self.samples[sample]["controlread1"] = os.path.join(
                                self.parameters["demultiplex"]["out_dir"],
                                f"Control_{sample}.sampling.r1.fastq",
                            )
                            self.samples[sample]["controlread2"] = os.path.join(
                                self.parameters["demultiplex"]["out_dir"],
                                f"Control_{sample}.sampling.r2.fastq",
                            )
                        else:
                            self.samples[sample]["read1"] = os.path.join(
                                self.parameters["demultiplex"]["out_dir"],
                            ...
                            self.samples[sample]["controlread1"] = os.path.join(
                                self.parameters["demultiplex"]["out_dir"],
                                f"Control_{sample}.r1.fastq",
                            )
                            self.samples[sample]["controlread2"] = os.path.join(
                                self.parameters["demultiplex"]["out_dir"],
                                f"Control_{sample}.r2.fastq",
                            )

                    sample_alignment_path = os.path.join(
                        self.parameters["analysis_folder"],
                        "aligned",
                        sample + ".dedup.bam",
                    )
    ...
    def identifyOfftargetSites(self):
        ...
        try:
            ...
            for sample in self.samples:
                try:
                    # default input
                    if "aligned" not in self.samples[sample]:
                        self.samples[sample]["aligned"] = os.path.join(
                            self.parameters["analysis_folder"],
                            "aligned",
                            sample + ".dedup.bam",
                        )
                        self.samples[sample]["controlaligned"] = os.path.join(
                            self.parameters["analysis_folder"],
                            "aligned",
                            "Control_" + sample + ".dedup.bam",
                        )
                    ...
                    self.identified[sample] = os.path.join(
                        self.parameters["analysis_folder"],
                        "identified",
                        sample + "_identifiedOfftargets.txt",
                    )

                    identifyOfftargetSites.analyze(
                    ...
                    identifyOfftargetSites.analyze(
                        self.samples[sample.replace("Control_", "")]["controlaligned"],
                        self.parameters["reference_genome"],
                        self.identified[sample],
                        annotations,
                        self.parameters["window_size"],
        ...
        for sample in self.samples:
            try:
                out = os.path.join(
                    self.parameters["analysis_folder"],
                    "identified",
                    sample + "_identifiedOfftargets.rmblck.txt",
                )
                filterBlackList(
                    self.parameters["bedtools"],
                    self.identified[sample],
                ...
                out = os.path.join(
                    self.parameters["analysis_folder"],
                    "identified",
                    sample + "_identifiedOfftargets.rmblck.with_control_counts.txt",
                )
                filterControl(
                    self.parameters["bedtools"],
                    self.identified[sample],
                    self.identified["Control_" + sample],
                    out,
                )
                logger.info(
                    "Finished background filtering for {0} sample".format(sample)
                )
    ...
    def visualize(self):
        ...
        for sample in (
            self.samples
        ):  ## 3/6/2020 Yichao solved: visualization stopped when one sample failed
            try:
                ...
                try:
                    ...
                    visualizeOfftargets(
                        ...
                        outfile,
                        title=sample,
                        PAM=self.parameters["PAM"],
                        genome=self.parameters["genome"],
                        refseq_names=self.parameters["refseq_names"],
                    )
                except:
                    visualizeOfftargets(
                        infile,
...
def parse_args():
    ...
    subparsers = parser.add_subparsers(
        description="Two run mode: main or parallel",
    ...
    parallel_parser = subparsers.add_parser(
        "parallel",
        help="submit a job for each sample specified in the YAML file. Tested in the LSF system. May not work in other job management systems.",
    )
    parallel_parser.add_argument(
        "--manifest", "-m", help="Specify the manifest Path", required=True
    )
    parallel_parser.add_argument(
        "--sample",
        "-s",
        help="Specify sample to process (default is all)",
        default="all",
    )
    parallel_parser.add_argument(
        "--lsf",
        "-l",
        help="Specify LSF CMD",
        default='bsub -n 12 -R "span[hosts=1] rusage[mem=12000]" -P GUIDEseqV2 -q standard -o HPC_parallel_log/GUIDEseqV2_{Sample_Name}_%J.log',
    )
    parallel_parser.add_argument(
        "--step",
        help="Specify which steps of pipepline to run (demultiplex, align, identify,visualize)",
        default="demultiplex+align+identify+visualize",
    )
    parallel_parser.add_argument(
        "--overwrite",
        help="overwrite specifications in the yaml file",
        default=None,
        type=yaml.load,
    )

    main_parser = subparsers.add_parser(
        "main", help="Run a single step or a series of steps for one or all samples"
    )
    ...
    main_parser.add_argument(
        "--step",
        help="Specify steps, demultiplex, align, identify,visualize, order does not matter",
        default="demultiplex+align+identify+visualize",
    )
    main_parser.add_argument(
        "--overwrite",
    ...
    init_parser = subparsers.add_parser(
        "init", help="initialize a default yaml in the current folder"
    )
            ...
            try:
                ...
                for sample in g.samples:
                    if "identified" in g.samples[sample]:
                        ...
                        g.identified["Control_" + sample] = g.samples[sample][
                            "controlidentified"
                        ]
                    else:
                        g.identified[sample] = os.path.join(
                            g.parameters["analysis_folder"],
                            "identified",
                            sample + "_identifiedOfftargets.rmblck.txt",
                        )
                        sample = "Control_" + sample
